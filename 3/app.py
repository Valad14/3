...
TYPE_EXPLANATIONS = {
    "Не указано": "тип нужно выбрать вручную",
    ...
    for i, (base_word, aligned_word) in enumerate(zip(base_words, aligned_words)):
        ...
        if aligned_word:
            target_elem = E.w(
                ...
                type="target",
                variant=variant_types.get(i, "Не указано"),
            )
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for o_name in data["others_list"]:
            ...
            for i, _ in enumerate(base_words):
                ...
                variant_types[i] = str(edited_df.iloc[i][type_col]) if type_col in edited_df.columns else "Не указано"
...
with st.expander("Подробная инструкция пользователя", expanded=False):
    st.markdown(
        ...
        Столбцы **Тип (...)** оставлены в таблице. По умолчанию программа не
        навязывает автоматическую классификацию: для найденных соответствий стоит
        **Не указано**, а для отсутствующих слов — **Пропуск**. Тип можно выбрать
        вручную прямо в таблице.

        В боковой панели можно включить режим **Автоматически предложить типы** —
        тогда программа заполнит типы как подсказку, но эти значения всё равно
...
with st.sidebar:
    ...
    auto_type_suggestions = st.checkbox(
        ...
        value=False,
        help="Если выключено, типы остаются в таблице, но для найденных соответствий ставится «Не указано» и их можно выбрать вручную.",
    )
            ...
            for i, base_word in enumerate(base_words):
                ...
                for other_name in others:
                    ...
                    if matched_word is None:
                        variant_type, reason = "Пропуск", "нет соответствующего слова в выбранном списке"
                    elif auto_type_suggestions:
                        variant_type, reason = classify_variant(base_word, matched_word)
                    else:
                        variant_type, reason = "Не указано", "тип не задан автоматически; выберите его вручную"
...
if "comp_df" in st.session_state:
    ...
    type_column_config = {
        col: st.column_config.SelectboxColumn(
            ...
            required=True,
            help="Тип можно выбрать или исправить вручную.",
        )
