...
# Определение типа разночтений. Таблица заполняется автоматически и
# использует только эти пять категорий. Пустых значений в столбцах «Тип» нет.
VARIANT_TYPES = [
    "графические",
    "фонетические",
    "морфологические",
...
TYPE_EXPLANATIONS = {
    "графические": "различие в написании: буква, титло, надстрочный знак, орфографический вариант; также используется для точных совпадений, чтобы таблица была заполнена полностью",
    ...
    "лексические": "использована другая лексема, другое слово или слово отсутствует в одном из списков",
}
...
def classify_variant(main_word: dict | None, witness_word: dict | None) -> tuple[str, str]:
    """Автоматически выбирает один из пяти типов и не возвращает пустое значение.

    В таблице нет категорий «Идентично», «Пропуск» и «Не указано», поэтому:
    - точное совпадение записывается как «графические» с пояснением, что видимого
      разночтения нет;
    - отсутствие соответствующего слова записывается как «лексические»;
    - «синтаксические» автоматически ставятся только в очевидных позиционных
      случаях, остальные такие места можно исправить вручную.
    """
    if not main_word or not witness_word:
        return "лексические", "нет соответствующего слова; пропуск записан как лексическое разночтение"
    ...
    # 1. Если форма совпадает, но XML-грамматика или лемма отличаются,
    # это полезнее показать как морфологическое/лексическое разночтение.
    if same_surface:
        if same_lemma(main_word, witness_word) and both_have_morph(main_word, witness_word) and not same_morph(main_word, witness_word):
            return (
                "морфологические",
                f"форма совпадает, но грамматика различается: "
        ...
        return "графические", "точное совпадение; видимого разночтения нет, поле заполнено автоматически"

    # 2. Та же лемма + разные грамматические признаки = морфология.
    if same_lemma(main_word, witness_word) and both_have_morph(main_word, witness_word) and not same_morph(main_word, witness_word):
        return (
            "морфологические",
    ...
    # 3. Та же лемма + совпадение после снятия титл/надстрочных знаков = графика.
    ...
    # 4. Разные леммы, но форма совпала после орфографической нормализации = графика.
    if same_normalized_graphics:
        return "графические", "формы совпали после орфографической нормализации"

    # 5. Разные леммы, но фонетически форма близка = фонетика.
    ...
    # 6. Очень близкие написания без совпадения леммы чаще всего являются графикой.
    if sim >= 0.82:
        return "графические", f"формы очень близки графически, сходство {sim:.2f}"

    # 7. Всё остальное считается лексической заменой.
...
def looks_syntactic_by_position(row_index: int, matched_word: dict | None, matches: dict[int, dict | None]) -> bool:
    ...
    Синтаксическое разночтение связано с порядком/позицией слова, поэтому здесь
    проверяется, выбивается ли позиция найденного слова из локального выравнивания.
    Эвристика специально строгая, чтобы не помечать обычные лексические замены как
    синтаксис.
    """
    if not matched_word:
        return False
...
def classify_variant_with_context(
    ...
    # Синтаксис предлагаем только для близких/тех же слов, которые явно сдвинуты
    # относительно соседних выровненных слов. Пропуски и лексические замены не
    # переводятся в синтаксис автоматически.
    if (
        main_word
        and witness_word
...
with st.expander("Подробная инструкция пользователя", expanded=False):
    st.markdown(
        ...
        **синтаксические**, **лексические**.
        ...
        Столбцы **Тип (...)** заполняются автоматически для всех строк и
        используют только эти пять категорий. Пустых значений, `Не указано`,
        `Идентично` и `Пропуск` в типах нет.

        Для строк без соответствующего слова программа ставит **лексические**.
        Для точных совпадений программа ставит **графические** с пояснением,
        что видимого разночтения нет: так таблица остается полностью заполненной.
...
if "comp_df" in st.session_state:
    ...
    type_column_config = {
        col: st.column_config.SelectboxColumn(
            ...
            required=True,
            help="Тип разночтения выбирается автоматически, но его можно исправить вручную: графические, фонетические, морфологические, синтаксические или лексические.",
        )
        for col in type_cols
    }
    disabled_cols = [col for col in table_df.columns if col not in type_cols]

    st.caption("Типы заполняются автоматически для всех строк: графические, фонетические, морфологические, синтаксические, лексические.")
