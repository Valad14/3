...
# Значения в столбцах «Тип». Таблица заполняется автоматически полностью:
# пять типов разночтений + два служебных значения для точных совпадений и пропусков.
VARIANT_TYPES = [
    "графические",
    "фонетические",
    "морфологические",
...
def classify_variant(main_word: dict | None, witness_word: dict | None) -> tuple[str, str]:
    ...
    Используются семь значений: «идентично», «пропуск» и пять типов
    разночтений: графические, фонетические, морфологические,
    ...
    # 1. Полное совпадение: форма, лемма и грамматическая разметка одинаковые.
    ...
    # 2. Если форма совпадает, но XML-грамматика или лемма отличаются,
    ...
    # 3. Та же лемма + разные грамматические признаки = морфология.
    if same_lemma(main_word, witness_word) and both_have_morph(main_word, witness_word) and not same_morph(main_word, witness_word):
        return (
            "морфологические",
    ...
    # 4. Та же лемма + совпадение после снятия титл/надстрочных знаков = графика.
    ...
    # 5. Разные леммы, но форма совпала после орфографической нормализации = графика.
    if same_normalized_graphics:
        return "графические", "формы совпали после орфографической нормализации"

    # 6. Разные леммы, но фонетически форма близка = фонетика.
    ...
    # 7. Очень близкие написания без совпадения леммы чаще всего являются графикой.
    if sim >= 0.82:
        return "графические", f"формы очень близки графически, сходство {sim:.2f}"

    # 8. Всё остальное считается лексической заменой.
...
def classify_variant_with_context(
    ...
    # относительно соседних выровненных слов. Пропуски, точные совпадения и
    # лексические замены не переводятся в синтаксис автоматически.
    if (
        main_word
        and witness_word
...
def export_aligned_xml(
    ...
    for i, (base_word, aligned_word) in enumerate(zip(base_words, aligned_words)):
        ...
        if aligned_word:
            ...
            ab.append(E.w("---", type="missing", variant=selected_variant))
        ab.append(E.pc(" "))
...
with st.expander("Подробная инструкция пользователя", expanded=False):
    st.markdown(
        ...
        **синтаксические**, **лексические**, **пропуск**, **идентично**.

        Столбцы **Тип (...)** заполняются автоматически для всех строк.
        Пустых значений и `Не указано` в типах нет.

        Для точных совпадений программа ставит **идентично**.
        Для строк без соответствующего слова программа ставит **пропуск**.
        Остальные случаи автоматически распределяются по типам разночтений.
...
if "comp_df" in st.session_state:
    ...
    type_column_config = {
        col: st.column_config.SelectboxColumn(
            ...
            required=True,
            help="Тип выбирается автоматически, но его можно исправить вручную: графические, фонетические, морфологические, синтаксические, лексические, пропуск или идентично.",
        )
