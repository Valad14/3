...
TYPE_EXPLANATIONS = {
    ...
    "Пропуск": "для слова основного списка не найдено соответствие в другом списке",
}
...
def classify_variant(main_word: dict | None, witness_word: dict | None) -> tuple[str, str]:
    """Автоматическая подсказка типа. Её можно исправить вручную в таблице."""
    ...
    if main_word["surface"] == witness_word["surface"]:
        ...
        return "Морфологическое", "форма совпадает, но лемма или XML-грамматика отличаются"
    ...
    if same_lemma(main_word, witness_word):
        ...
        return "Графическое", "лемма совпадает; различается написание"
...
def default_variant_type(main_word: dict | None, witness_word: dict | None) -> tuple[str, str]:
    """Безопасное автозаполнение: только очевидные случаи.

    Спорные различия не размечаются автоматически, чтобы программа не
    навязывала неверные значения «Морфологическое», «Графическое» и т. п.
    Пользователь выбирает их вручную в таблице.
    """
    if not main_word or not witness_word:
        return "Пропуск", "нет соответствующего слова в выбранном списке"
    ...
    if main_word["surface"] == witness_word["surface"] and same_morph(main_word, witness_word) and (
        ...
        return "Идентично", "очевидное совпадение: форма, лемма и XML-грамматика совпадают"

    return "", "тип не выбран автоматически; выберите его вручную"
...
with st.expander("Подробная инструкция пользователя", expanded=False):
    st.markdown(
        ...
        Столбцы **Тип (...)** оставлены в таблице. Программа автоматически
        заполняет только очевидные случаи: **Идентично** и **Пропуск**.
        Все спорные различия остаются пустыми, чтобы их можно было выбрать
        вручную прямо в таблице.

        В боковой панели можно включить режим **Автоматически предложить типы** —
        тогда программа заполнит спорные типы как подсказку, но эти значения
...
with st.sidebar:
    ...
    auto_type_suggestions = st.checkbox(
        ...
        value=False,
        help="Если выключено, программа заполняет только очевидные «Идентично» и «Пропуск», а спорные типы оставляет пустыми для ручного выбора.",
    )
...
if "comp_df" in st.session_state:
    ...
    type_column_config = {
        col: st.column_config.SelectboxColumn(
            ...
            required=False,
            help="Тип можно выбрать или исправить вручную. Пустая ячейка означает, что тип ещё не выбран.",
        )
