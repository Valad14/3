...
# Определение типа разночтений. В списке оставлены только категории,
# которые пользователь размечает как тип разночтения. Совпадение и пропуск
# не являются типами из этого определения, поэтому для них ячейка остается пустой.
VARIANT_TYPES = [
    "",
    "графические",
    "фонетические",
...
TYPE_EXPLANATIONS = {
    "": "тип разночтения не выбран или разночтения нет",
    ...
    "синтаксические": "различается порядок слов, связь слов или синтаксическая конструкция; обычно выбирается вручную",
...
def classify_variant(main_word: dict | None, witness_word: dict | None) -> tuple[str, str]:
    """Автоматическая подсказка одного из пяти типов разночтений.

    Подсказка не использует значения «Идентично» и «Пропуск», потому что их нет
    в пользовательском определении типа разночтений. Синтаксические варианты
    лучше отмечать вручную: по отдельной паре слов алгоритм не всегда может
    надежно увидеть изменение порядка слов или конструкции.
    """
    if not main_word or not witness_word:
        return "", "нет соответствующего слова; тип разночтения не выбран"
    ...
    if main_word["surface"] == witness_word["surface"]:
        if same_morph(main_word, witness_word) and (
            ...
            return "", "формы совпадают; разночтения нет"
        return "морфологические", "форма совпадает, но лемма или XML-грамматика отличаются"
    ...
    if same_lemma(main_word, witness_word):
        if main_word["orth"] == witness_word["orth"]:
            return "графические", "после снятия титл/надстрочных знаков формы совпадают"
        if (
        ...
        return "графические", "лемма совпадает; различается написание"
...
def default_variant_type(main_word: dict | None, witness_word: dict | None) -> tuple[str, str]:
    """По умолчанию тип не навязывается.

    Пользователь выбирает один из пяти типов вручную. Автоматические подсказки
    можно включить отдельным чекбоксом в боковой панели.
    """
    if not main_word or not witness_word:
        return "", "нет соответствующего слова; тип разночтения не выбран"

    if main_word["surface"] == witness_word["surface"] and same_morph(main_word, witness_word) and (
        not main_word.get("lemma_norm")
        or not witness_word.get("lemma_norm")
        or same_lemma(main_word, witness_word)
    ):
    ...
    return "", "выберите один из пяти типов разночтений вручную"
...
def export_aligned_xml(
    ...
    for i, (base_word, aligned_word) in enumerate(zip(base_words, aligned_words)):
        ...
        if aligned_word:
            ...
            selected_variant = str(variant_types.get(i, "") or "").strip()
            if selected_variant:
                target_attrs["variant"] = selected_variant
            target_elem = E.w(aligned_word["surface"], **target_attrs)
...
st.markdown(
    f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans:wght@400;600&family=Noto+Serif:wght@400;600&display=swap');
...
st.markdown(
    ...
    выравнивания с редактируемыми типами, контекстом, статистикой и экспортом.
    """
)
...
with st.expander("Подробная инструкция пользователя", expanded=False):
    st.markdown(
        """
        ### Определение типа разночтений

        В столбцах **Тип (...)** используются только пять категорий:
        **графические**, **фонетические**, **морфологические**,
        **синтаксические**, **лексические**.

        Совпадения и пропуски не записываются как тип разночтения, поэтому
        для них ячейка может оставаться пустой. Синтаксические разночтения
        обычно выбираются вручную, потому что они связаны с порядком слов
        или конструкцией, а не только с одной парой слов.

        В боковой панели можно включить режим **Автоматически предложить типы** —
        тогда программа заполнит часть типов как подсказку, но эти значения
        всё равно можно исправить вручную.
...
with st.sidebar:
    ...
    auto_type_suggestions = st.checkbox(
        "Автоматически предложить типы",
        value=False,
        help="Если выключено, типы остаются пустыми для ручной разметки. Если включено, программа предлагает только категории из определения: графические, фонетические, морфологические, синтаксические, лексические.",
    )
        ...
        with st.spinner("Синхронизация текстов..."):
            ...
            for i, base_word in enumerate(base_words):
                ...
                for other_name in others:
                    ...
                    if auto_type_suggestions:
                        variant_type, reason = classify_variant(base_word, matched_word)
                    else:
                        variant_type, reason = default_variant_type(base_word, matched_word)
            ...
            st.session_state.auto_type_suggestions = auto_type_suggestions
...
if "comp_df" in st.session_state:
    ...
    type_column_config = {
        col: st.column_config.SelectboxColumn(
            ...
            required=False,
            help="Выберите тип разночтения: графические, фонетические, морфологические, синтаксические или лексические. Пустая ячейка означает, что разночтения нет или тип ещё не выбран.",
        )
        for col in type_cols
    }
    disabled_cols = [col for col in table_df.columns if col not in type_cols]

    st.caption("Типы разночтений: графические, фонетические, морфологические, синтаксические, лексические.")
    ...
    edited_visible = st.data_editor(
        ...
        key="comparison_editor_types_no_filter",
    )
    ...
    with export_col3:
        if st.button("Новое сравнение", use_container_width=True):
            for key in ["comp_df", "others_list", "main_file", "base_words", "all_aligns", "auto_type_suggestions"]:
